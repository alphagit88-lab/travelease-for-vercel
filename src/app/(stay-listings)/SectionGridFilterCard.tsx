...
const SectionGridFilterCard: FC<SectionGridFilterCardProps> = ({
    ...
    return (
      <div className="grid grid-cols-1 gap-6 md:gap-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {Array.from({ length: 8 }).map((_, i) => (
          <div
            key={i}
            className="animate-pulse rounded-2xl bg-neutral-100 h-[340px] w-full"
          />
        ))}
      </div>
    );
  ...
  if (error || tours.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 px-4 rounded-3xl bg-neutral-50 border border-neutral-100 text-center w-full">
        ...
        <p className="text-neutral-500 font-medium max-w-md">
          Tours are currently unavailable. Please make sure the backend server is running.
        </p>
        <button
